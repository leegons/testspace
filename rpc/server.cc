...
private:
                      ...
                      google::protobuf::Closure* done)
    {
        ...
        sofa::pbrpc::RpcController* rpc_controller = static_cast<sofa::pbrpc::RpcController*>(controller);
        sofa::pbrpc::HttpResponseHead* headers = rpc_controller->GetHttpResponseHead();
        if (headers != nullptr) {
            headers->AddHeader("X-Kws-Code", "200");
            headers->SetStatus(200, "i'm fine");
        }
        int i = 10;
        while (i--) {
            SLOG(NOTICE, "ECHO(). wait %d/10 s to response", i);
            sleep(1);
        }
        response->set_message("echo message: " + request->message() + ", from server");
        done->Run();
    }
};
...
int main()
{
    ...
    // Start rpc server.
    if (!rpc_server.Start("0.0.0.0:23406")) {
